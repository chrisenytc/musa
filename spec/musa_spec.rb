...
describe Musa do
  ...
  before(:all) do
    ...
    File.unlink encrypted_file if File.exist?(encrypted_file)
    File.unlink decrypted_file if File.exist?(decrypted_file)
  end
  ...
  context 'throws a error on encrypt' do
    it 'when key is too short' do
      expect do
        Musa::Encryption.encrypt '', ''
      end.to raise_error(Musa::Error::ShortKey)
    end
  end
  ...
  context 'throws a error on encrypt file' do
    it 'when file is not found' do
      expect do
        Musa::Encryption.encrypt_file '',
                                      File.join(ROOT,
                                                'fixtures',
                                                'notfound.txt'),
                                      ''
      end.to raise_error(Musa::Error::FileNotFound)
    end
    it 'when key is too short' do
      expect do
        Musa::Encryption.encrypt_file '',
                                      File.join(ROOT, 'fixtures', 'test.txt'),
                                      ''
      end.to raise_error(Musa::Error::ShortKey)
    end
  end
  ...
  context 'throws a error on decrypt file' do
    it 'when file is not found' do
      expect do
        Musa::Decryption.decrypt_file '', File.join(ROOT,
                                                    'fixtures',
                                                    'encrypted_notfound.txt'),
                                      ''
      end.to raise_error(Musa::Error::FileNotFound)
    end
    it 'when key is too short' do
  ...
  context 'when is valid' do
    ...
    it 'decrypt a file' do
      ...
      decrypted_data = Musa::Decryption.decrypt_file key,
                                                     encrypted_path,
                                                     file_output
